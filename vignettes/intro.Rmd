...
__SC19019__ is a simple R package developed to compare the performance of R and R++ (implemented through the R package _Rcpp_) for the 'Statistical Computing' course. Two functions are considered, namely, _fr_ (find the bets k for kNN) and _fc_ (add). For fr function, R version is produced. Namely _StatCompR_. For fc function, C++ version is produced. Namely _StatCompC_.



The source R code for _fr_ is as follows:
...
Input x. Generate 100 standard normal numbers. Caculate the density estimate in x by knn method for k from 1 to 100. Find the best k for x.
...
Input three integer numbers. Return their sum.
