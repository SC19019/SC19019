...
p=11#x??ά??????????????????ά??
nn=0
generate_fixed_point<-function(j)#??????3_11??setting?Ĺ̶????Ե?
{
  x<-rep(1,j)
  x<-x-1
  for(i in 3:j)
...
generate_sample<-function(nn,m,p)#??????????????????????????????ÿ??ѭ???????ı?
{
  set.seed(1234+nn)
  mean<-rep(1,p)
  sigma<-diag(mean)
  ...
  w<-rbinom(m,1,0.5)
  z=cbind(mydata,w)
  return(z)#???????ɵ????????󣬻?δ??????Ӧ??��,p+1????w???????????Լ?û??????
}
#mydata=generate_sample(nn,m,p)  mydata??p???ǰ?????
#head(mydata,n=5)
generate_setting1_2<-function(mydata,m,p)#????????һ????Ӧy??p+2????y
{
  x<-mydata[,1:(p-1)]
  w<-mydata[,(p+1)]
  if(p==4)
  ...
  z=cbind(mydata,y)
  return(z)#???ص?һ???????е?x,w,y???ɵľ???
}
#????3_11?????µ???Ӧy?????Ұ?y??Ϊ??????????һ??
generate_setting3_11<-function(mydata,m,j)#mydata??j??Ϊ????????j+1????w
{
  y<-rep(1,m)
  y=y-1
  w<-mydata[,(j+1)]
...
estimate_setting_DNN<-function(zz,mm,ss,x,p)#????DNN?��?��
{
...
twoscaled_DNN<-function(zz,p,s,mm,xx)#??dΪ??ͬά??ʱ??ʱ????˫?߶?DNN?��?????ЧӦ,zz??x,w,y???ɵľ???
{
  z2<-data.frame(zz)
...
generate_CF_sample<-function(mydata,m,p,x.test)
{
  ...
  c.pred=predict(c.forest,t(x.test))#1*1?ľ???
  c.pre=c.pred[1,1]
  return(c.pre)
}
